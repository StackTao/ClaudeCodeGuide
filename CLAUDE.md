...
Static Chinese-language community knowledge base for Claude Code, deployed to GitHub Pages at `https://stacktao.github.io/ClaudeCodeGuide/`. Content is authored in Markdown under `content/`; the site is a zero-dependency static page that loads a generated JS data file.
...
1. **Authoring** — `content/*.md`. Each Markdown file maps to one "doc" registered in the `docs` array at `scripts/build-site.js:8`. Adding a new top-level doc requires editing that array (slug, title, description, file path) — the build does not auto-discover Markdown files.

2. **Build** — `scripts/build-site.js` reads each Markdown file, splits it into sections on `#` / `##` headings (code fences are respected, so `#` inside ``` blocks doesn't trigger a split), slugifies headings (including CJK punctuation stripping) with collision suffixes, and emits `dist/docs-data.js` as a single `window.DOCS = [...]` assignment. It also copies `src/{index.html,app.js,styles.css,404.html}`, plus root `README.md` and `.nojekyll`, into `dist/`. `dist/` is fully rebuilt (`fs.rmSync` then `mkdir`) on every run.
...
- The first heading of a file always becomes the doc's "overview" section regardless of its slug.
